...
const UpdateUser = () => {
    ...
    const fetchUserData = async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/users/getbyid/${id}`);
    ...
    const submitForm = async (values) => {
        try {
            const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/users/update/${id}`, values);
            if (res.status === 200) {
